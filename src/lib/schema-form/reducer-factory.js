...
import {setIn} from '../common/utils/immutable-util';
import memoize from 'lodash/memoize';
import Ajv from 'ajv';
import {deserializeDraftJs, toPojo} from './util/draftjs';
...
function ignoreEditorState(obj) {
  return obj instanceof EditorState ? {} : obj;
}
...
function registerForm(state, {meta, payload}) {
  ...
  const initialData = deserializeDraftJs(schema, payload.initialData || {});
...
function validate(state, {meta}) {
  ...
  if (schema) {
    ...
    const valid = validate(toPojo(data, ignoreEditorState));
    if (!valid) {
