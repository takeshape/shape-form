import {takeLatest, take, put, all, select, delay} from 'redux-saga/effects';
...
export function* submitFormSaga(action) {
  ...
  if (hasErrors) {
    ...
    if (action.callback || withValues) {
      ...
      const structuralChanges = yield select(getStructuralChanges, {
        formName
      });
...
export default function* root() {
  yield all([
    takeLatest([...pauseChanges, SUBMIT_FORM, BLUR_FIELD, ADD_ARRAY_ITEM], validateOnChange),
    takeLatest(SUBMIT_FORM, submitFormSaga)
  ]);
}
